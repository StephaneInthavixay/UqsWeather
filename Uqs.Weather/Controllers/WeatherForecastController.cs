...
public class WeatherForecastController : ControllerBase
{
    ...
    private readonly IRandowWrapper _randowWrapper;
    ...
    public WeatherForecastController(ILogger<WeatherForecastController> logger, IClient client, INowWrapper nowWrapper, IRandowWrapper randowWrapper)
    {
        _logger = logger;
        _client = client;
        _nowWrapper = nowWrapper;
        _randowWrapper = randowWrapper;
    }
    ...
    public IEnumerable<WeatherForecast> GetRandom()
    {
        ...
        for (int i = 0; i < wfs.Length; i++)
        {
            ...
            wf.TemperatureC = _randowWrapper.Next(-20, 55);
            wf.Summary = MapFeelToTemp(wf.TemperatureC);
        }
    ...
    private string MapFeelToTemp(int temperateurC)
    {
        if (temperateurC <= 0)
            return Summaries.First();

        int summariesIndex = (temperateurC / 5) + 1;
